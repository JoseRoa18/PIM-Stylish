    ...
    <div className="space-y-8">
        ...
        <div className="space-y-3">
          {LIVE_CHANNELS.map((c) => (
            <Link
              key={c.id}
              to={`/syndication/${c.id}`}
              className="flex items-center gap-4 rounded-2xl border border-outline-variant bg-surface-container-lowest [.dark_&]:bg-surface-container-low px-6 py-5 hover:bg-surface-container hover:border-outline transition-colors group"
            >
              ...
              <div className="text-right flex-shrink-0 hidden sm:block">
                {stats[c.id] ? (
                  ...
                  <div className="animate-pulse" aria-hidden="true">
                    <div className="h-5 w-16 rounded bg-surface-container-highest ml-auto" />
                    <div className="h-3.5 w-24 rounded bg-surface-container-highest ml-auto mt-1" />
                  </div>
                )}
              </div>
      ...
      <section className="space-y-3">
        ...
        {fileChannels === null ? (
          ...
          <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest [.dark_&]:bg-surface-container-low px-6 py-8 text-center">
            <div className="w-10 h-10 rounded-full bg-tertiary-container text-on-tertiary-container mx-auto mb-3 flex items-center justify-center">
              <FileSpreadsheet className="w-5 h-5" />
            </div>
            <p className="text-body-md text-on-surface mb-1">No file channels yet</p>
            <p className="text-body-sm text-on-surface-variant">
              Upload a marketplace template and it will appear here.
            </p>
            <Link to="/templates" className="inline-block mt-3 text-body-sm text-primary font-medium hover:underline">
              Go to Templates
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {fileChannels.map((f) => (
              <Link
                key={f.marketplace}
                to="/templates"
                className="flex items-center gap-3 rounded-xl border border-outline-variant bg-surface-container-lowest [.dark_&]:bg-surface-container-low px-4 py-3 hover:bg-surface-container hover:border-outline transition-colors"
              >
