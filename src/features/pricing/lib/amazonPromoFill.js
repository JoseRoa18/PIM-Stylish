...
function locate(grid) {
  for (let r = 0; r < Math.min(HEADER_SCAN_ROWS, grid.length); r++) {
    ...
    const technical = next.some((h) => /^(item_sku|sku|sale_price|sale_from_date)$/.test(h));
...
export async function fillAmazonPromoTemplate(template, promotion, channel) {
  ...
  if (toAppend.length) {
    let rowsXml = '';
    for (const [idx, o] of toAppend.entries()) {
      const rn = lastRow + 1 + idx;
      rowsXml += `<row r="${rn}">` + [...cellsFor(rn, o).entries()].sort((a, b) => a[0] - b[0]).map(([, x]) => x).join('') + '</row>';
    }
